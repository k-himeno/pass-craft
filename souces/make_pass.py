...
class PasswordGenerator:
    def __init__(self, csv_path):
        ...
        self.master1, self.master2 = self.input_master()

    def make_password(
        self, master1, master2, seed="1234", plen=20, char="ans", rm_cha="", start=1, upper=False, lower=False, **kwargs
    ):
        """パスワードを生成する関数
    ...
    def input_master(self):
        ...
        while flag:
            ...
            print("マスター2を入力: ")
            master2 = input()
            _ = self.make_password(master1, master2, seed="1234", plen=20, char="ans", rm_cha="", start=1)
            print("マスターパスワードは正しいですか？(y/n)")
            flag = input() == "n"
        return master1, master2

    def get_input_site_info(self):
        """サイトの情報を入力する"""
    ...
    def append_dict_to_csv(self, data_dict):
        try:
    ...
    def run(self):
        site_info = self.get_input_site_info()
        password_data = self.make_password(self.master1, self.master2, **site_info)
        print(password_data["password"])
        self.append_dict_to_csv(site_info)


if __name__ == "__main__":
    path = "hash_key.csv"
    generator = PasswordGenerator(path)
    generator.run()
